<template>
	<div class="card bordered">
		<header><h2 class="title is-5">Tourney Points</h2></header>
		<div class="card-content">
			...
			<table class="table leaderboard is-hoverable">
				...
				<template v-else>
				<tr v-for="(player, index) of players" :key="`tourney-${player.id}`">
					<td class="rank">
					...
					<td class="td-points">{{ player.points |formatted }}</td>
				</tr>
				</template>
				</tbody>
			</table>
...
export default Vue.extend({
	...
	async mounted () {
		await this.getTourneyLadder()
	},
	methods: {
		async getTourneyLadder () {
			this.loading = true
			this.players = await this.$strapi.find('ladders/tourney', { _limit: 10 })
			this.loading = false
		},
	},
})
